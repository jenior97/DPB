...
class Info_Sampler:
	def __init__(self, phi_num):
		...
		self.a = [] # responses (-1 for A, 1 for B)
	...
	def logp(self, i, w, delta=0):
		...
		if a != 0:
			phi_select = phi_B if a < 0 else phi_A
			phi_nonsel = phi_A if a < 0 else phi_B
	...
	def sample_given_delta(self, sample_count, query_type, delta, burn=1000, thin=50, step_size=0.1):
		...
		for _ in range(burn + thin*sample_count):
			...
			if np.log(np.random.rand()) < new_logprob - old_logprob:
				...
				x = np.vstack((x,x[-1]))
			sample_logprobs.append(old_logprob)
		x = x[burn+thin-1::thin]
		sample_logprobs = np.array(sample_logprobs[burn+thin-1::thin])
		return x, delta * np.ones((x.shape[0],)), sample_logprobs
