...
class batch_active_PBL(PBL_model):
    ...
    def __init__(self, simulation_object, batch_active_params, env='simulated'):
        ...
        if self.method =='information':
            ...
            self.w_samples, self.delta_samples, self.sample_logprobs = self.w_sampler.sample_given_delta(self.M, 'strict', 0)
        elif self.method == 'max_regret':
            self.w_sampler = Info_Sampler(self.simulation_object.num_of_features)
            self.w_samples, self.delta_samples, self.sample_logprobs = self.w_sampler.sample_given_delta(self.M, 'strict', 0)
        else:
    ...
    def update_param(self, t):
        ...
        if self.method == 'information':
            self.w_sampler.feed(self.action_s1, self.action_s2, self.reward_s)
            self.w_samples, self.delta_samples, self.sample_logprobs = self.w_sampler.sample_given_delta(self.M, 'strict', 0)
    ...
    def select_batch_actions(self, step, b):
        ...
        def avoid_env():
            ...
            given_features = self.predefined_features
            ...
            if step == 0:
                ...
                if self.method == 'information':
                    ...
                    feature_idx1, feature_idx2 = run_algo(self.method, self.simulation_object, self.w_samples, b, self.B, self.delta_samples, self.sample_logprobs)
                    selected_actions = given_features[feature_idx1] - given_features[feature_idx2]
                    return selected_actions, feature_idx1, feature_idx2
                else:
        ...
        def benchmark_env():
            ...
            if step == 0:
                ...
                given_actions = self.PSI
                random_initialize = np.random.randint(0, len(given_actions), b)
                ...
                if self.method == 'information':
                    ...
                    inputA_set, inputB_set = run_algo(self.method, self.simulation_object, self.w_samples, b, self.B, self.delta_samples, self.sample_logprobs)
                else:
