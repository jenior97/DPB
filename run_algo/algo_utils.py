...
def timevarying_true_w(task, features_d):
    ...
    if task == "tosser":
        ...
        true_w[0][0] = np.random.uniform(-1, 1)
        true_w[0][1] = np.random.uniform(-1, 1)
        true_w[0][2] = np.random.uniform(-1, 1)
        true_w[0][3] = np.random.uniform(-1, 1)
        true_w[0] = true_w[0]/np.linalg.norm(true_w[0])

        # Random sampling
        target_w = np.random.rand(features_d)
        target_w[0] = np.random.uniform(-1, 1)
        target_w[1] = np.random.uniform(-1, 1)
        target_w[2] = np.random.uniform(-1, 1)
        target_w[3] = np.random.uniform(-1, 1)
        target_w = target_w/np.linalg.norm(target_w)

        for i in range(1,9):
            increments = (target_w - true_w[0])/9
            intermediate_w = true_w[0] + increments * i
            true_w.append(intermediate_w / np.linalg.norm(intermediate_w))
